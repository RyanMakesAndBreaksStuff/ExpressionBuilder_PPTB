...
describe('shared builder UI', () => {
  ...
  it('search filters fields by label', async () => {
    ...
    await user.type(screen.getByLabelText('Search fields'), 'due');
  ...
  it('selecting a condition row updates the inspector', async () => {
    const user = userEvent.setup();
    render(<ExpressionBuilderShell adapter={createAdapter()} />);

    await user.click(screen.getByRole('option', { name: /Approver contains/i }));

    expect(screen.getByRole('heading', { name: 'Edit rule' })).toBeInTheDocument();
    expect(screen.getByLabelText('Field')).toHaveValue('Approver');
  ...
  it('case-insensitive fix wraps both sides in toLower()', async () => {
    ...
    await user.click(screen.getByRole('option', { name: /Approver contains/i }));
    await user.click(screen.getByRole('button', { name: 'Wrap both sides in toLower()' }));
